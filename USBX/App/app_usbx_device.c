...
UINT MX_USBX_Device_Stack_Init(void) {
  ...
  device_framework_high_speed = USBD_Get_Device_Framework_Speed(
      USBD_HIGH_SPEED, &device_framework_hs_length);
  ...
  usbx_debug_phase = 20;
  {
    UINT init_ret = ux_device_stack_initialize(
    ...
    usbx_debug_phase = 200 + init_ret; /* e.g. 200=OK, 201=error, etc. */
    if (init_ret != UX_SUCCESS) {
  ...
  cdc_acm_configuration_number =
      USBD_Get_Configuration_Number(CLASS_TYPE_CDC_ACM, 0);
  ...
  if (ux_device_stack_class_register(
  ...
  /* Start the USB controller — required to enable interrupts and D+ pullup */
...
UINT MX_USBX_Device_Stack_DeInit(void) {
  ...
  ux_dcd_stm32_uninitialize((ULONG)USB_OTG_FS, (ULONG)&hpcd_USB_OTG_FS);
